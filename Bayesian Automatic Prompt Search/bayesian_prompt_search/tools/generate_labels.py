...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    label_word_lists: list = field(
        default_factory = lambda:['a']
    )

    label_word_list_scores: list = field(
        default_factory= lambda:['a']
    )
    ...
    output_score_file: str = field(
        default='out_score',
        metadata={"help":"Output file to write score of each mapping"}
    )
...
def main():
    ...
    # Find best labels.
    label_pairings, score_pairing = find_labels(
        model=trainer.model,
        train_logits=logits,
        train_labels=labels,
    ...
    with open(data_args.output_file, mode) as f:
        with open(data_args.output_score_file, mode) as f2:
            # Juncheng: New Code for Reporting Score of Each Mapping
            for i, pairing in enumerate(label_pairings):
                ...
                f2.write(json.dumps(mapping) + "\n")
                f2.write(f'{score_pairing[i]}\n')
