...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        self.label_list = self.processor.get_labels()  # self.label_list = ['0','1']
        self.num_labels = len(self.label_list)
        
        if args.prompt:
            # Juncheng: The following block of codes cleans label words and returns 
            # 1. self.label_to_word ( a dictionary with label as keys and token of the corresponding word as values)
            # 2. self.label_word_list

            assert args.mapping is not None
            # self.label_to_word is a dictionary. e.g. {"0": "pathetic", "1": "irresistible"}
        ...
        # ============ New Code for Bayesian Method ================
        if args.prompt:

            # Process the list of label mappings for Bayesian method 
            if args.Bayesian:
                self.label_to_word_lists=[]
                self.label_word_list_lists=[]
                # args.mapping is a list of label-word mapping
                for i in range(len(args.label_to_word_lists)):
                    self.label_to_word = eval(args.label_to_word_lists[i]) 
                    ...
                    self.label_to_word_lists.append(self.label_to_word)
                    self.label_word_list_lists.append(self.label_word_list)
