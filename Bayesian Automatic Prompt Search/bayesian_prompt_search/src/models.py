...
import sys
sys.path.append('/hpc/home/jd420/MetaLearn/LM-BFF/venv/lib/python3.8/site-packages')
...
class RobertaForPromptFinetuning(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # ====== Bayesian Method Attributes ======
        self.label_word_list_lists = None
        self.label_word_list_scores = None
        # ========================================
    ...
    def forward(
        ...
        if self.data_args.Bayesian:
            
            logits_list = []
            for i in range(len(self.label_word_list_lists)):
                label_word_list = self.label_word_list_lists[i]
                logits = []
                for label_id in range(len(label_word_list)):
                    logits.append(prediction_mask_scores[:, label_word_list[label_id]].unsqueeze(-1))
                logits = torch.cat(logits, -1)
                logits_list.append(math.exp(self.label_word_list_scores[i]/self.data_args.tau)*logits)
            
            # weighted logits according to Bayesian probability
