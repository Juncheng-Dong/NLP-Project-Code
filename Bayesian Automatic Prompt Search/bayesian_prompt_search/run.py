...
import sys
sys.path.insert(0,'/hpc/home/jd420/Projects/LM-BFF/venv/lib/python3.8/site-packages')
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    Bayesian: bool = field(
        default=False,
        metadata={"help":"whether to use Bayesian method"}
    )

    label_to_word_lists: list = field(
        default_factory = lambda:['a']
    )

    label_to_word_lists_scores: list = field(
        default_factory= lambda:['a']
    )

    tau: float = field(
        default = 1.0,
        metadata={"help":"regularizing hyper-parameter for Bayesian method"}
    )
    ...
    mapping_score_path: str = field(
        default=None,
        metadata={"help": "Path to a txt file that stores all the label word mappings and their corresponding scores"}
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        # Juncheng: split arguments into three categories - model, data, training
    ...
    if data_args.prompt:
        if data_args.prompt_path is not None:
            ...
            # =========== New Code for Using All Label Mappings =============
            if data_args.mapping_score_path is not None:
                with open(data_args.mapping_score_path) as f:
                    mapping_list = []
                    score_list = []
                    for line in f:
                        line = line.strip()
                        line = line.split(maxsplit=1)
                        mapping_list.append(line[1])
                        score_list.append(float(line[0]))

                data_args.label_to_word_lists = mapping_list
                data_args.label_to_word_lists_scores = score_list

                logger.info('********')
                logger.info('Finish reading list of mappings and their scores')
                logger.info('********\n')
            # ===============================================================
    ...
    # Juncheng: Pass the label word list into the Transformer model
    # Pass dataset and argument information to the model
    if data_args.prompt:
        # model.label_word_list is the a list of tokens correponding to labels. 
        model.label_word_list = torch.tensor(train_dataset.label_word_list).long().cuda()

    # Juncheng: Adding new arguments to the model
    if data_args.Bayesian:
        model.label_word_list_lists = [torch.tensor(l).long().cuda() for l in train_dataset.label_word_list_lists]
        model.label_word_list_scores = data_args.label_to_word_lists_scores
    ...
    # Juncheng: Create a Trainer object
